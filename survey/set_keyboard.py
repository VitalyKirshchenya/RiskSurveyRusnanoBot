from admin.settings import MATURITY_LEVEL
from admin.settings import KEYBOARD_MENU
import sqlite3
...
def generate_country_kb(user):
    if user.language_ == 'ru':
        column = 1
    elif user.language_ == 'de':
        column = 3
    else:
        column = 2
    countries_rk = []
    try:
        db = sqlite3.connect('survey/GeoMySql/geodata5.db')
        cursor = db.cursor()
        cursor.execute("SELECT * FROM countries ORDER BY title_en")
        country_id = cursor.fetchall()
        for i in range(0,len(country_id)):
            countries_rk.append([country_id[i][column]])
    except sqlite3.Error as error:
        print(error)
    return countries_rk


def generate_city_kb(user):

    country = user.country_
    try:
        db = sqlite3.connect('survey/GeoMySql/geodata5.db')
        cursor = db.cursor()
        cursor.execute("SELECT country_id FROM countries WHERE title_en=? OR title_ru=? OR title_de=?", (country, country, country,))
        country_id = cursor.fetchone()[0]
        cursor.execute("SELECT title_en FROM cities WHERE country_id=?", (country_id,))
...
def get_keyboard(choice, user, kb_presets=None):
    if choice == []:
    ...
    if choice[0][0] == 'KB_CITY':
        return ReplyKeyboardMarkup(generate_city_kb(user))
    if choice[0][0] == 'KB_COUNTRY':
        return ReplyKeyboardMarkup(generate_country_kb(user))
...
def get_keyboard_inline(language):
    keyboard = []
    for i in range(0, 5):
        callback_button = [
            InlineKeyboardButton(text=MATURITY_LEVEL[i][language], callback_data=MATURITY_LEVEL[i][language])]
        keyboard.append(callback_button)
    # keyboard.append([InlineKeyboardButton(text=BACK_BUTTON[language], callback_data='back')])

    return InlineKeyboardMarkup(keyboard)


def keyboard_menu(language):
    keyboard = []
    for i in range(0, 5):
        menu_buttons = [KEYBOARD_MENU[i][language]]
